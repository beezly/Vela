...
class Visualizer():
    def __init__(self, board):
        ...
        self.effects = {"Scroll":self.visualize_scroll,
                        ...
                        "Visualight":self.visualize_visualight,
        ...
        self.dynamic_effects_config = {"Energy":[["blur", "Blur", "float_slider", (0.1,4.0,0.1)],
                                   ...
                                   "Wavelength":[["color_mode", "Color Mode", "dropdown", config.settings["gradients"]],
                                       ...
                                       "Scroll":[["sensitivity", "Sensitivity", "float_slider", (0.1,4.0,0.1)],
                                                 ...
                                                 ["speed", "Speed", "float_slider", (0,6,0.0005)]],
                                        "Power":[["color_mode", "Color Mode", "dropdown", config.settings["gradients"]],
                                                 ["color_flash", "Spark Color ", "dropdown", config.settings["colors"]],
                                   ...
                                   "Visualight":[["blur", "Blur", "float_slider", (0.1,4.0,0.1)],
                                                 ["saturation", "Saturation", "float_slider", (0.1,4.0,0.1)],
                                                 ["contrast", "Contrast", "float_slider", (0.1,10.0,0.1)],
                                                 ["gamma_r", "Gamma Red", "float_slider", (0.1,5.0,0.1)],
                                                 ["gamma_g", "Gamma Green", "float_slider", (0.1,5.0,0.1)],
                                     ...
                                     "Gradient":[["color_mode", "Color Mode", "dropdown", config.settings["gradients"]],
                                         ...
                                         "Fade":[["color_mode", "Color Mode", "dropdown", config.settings["gradients"]],
                                                 ["roll_speed", "Fade Speed", "float_slider", (0,8,0.0005)],
                                                 ["reverse", "Reverse", "checkbox"]],
    ...
    def get_vis(self, y, audio_input):
        effect = config.settings["devices"][self.board]["configuration"]["current_effect"]
...
#   return np.concatenate((p[:, ::-1], p), axis=1)
#endold
        y = y**config.settings["devices"][self.board]["effect_opts"]["Scroll"]["sensitivity"]
        #signal_processers[self.board].gain.update(y)   #Should try out old method for comparison
        #y /= signal_processers[self.board].gain.value  #Should try out old method for comparison
        #y *= 255.0                                     #Should try out old method for comparison
        ...
        current_scroll += config.settings["devices"][self.board]["effect_opts"]["Scroll"]["speed"] + 1
        speed = int( current_scroll )
    ...
    def visualize_wave(self, y):
        ...
        if self.current_freq_detects["beat"]:
            ...
            for i in range(self.wave_wipe_count):
                output[0][i]=config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Wave"]["color"]][0]
                output[0][-i]=config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Wave"]["color"]][0]
                output[1][i]=config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Wave"]["color"]][1]
                output[1][-i]=config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Wave"]["color"]][1]
                output[2][i]=config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Wave"]["color"]][2]
                output[2][-i]=config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Wave"]["color"]][2]
            ...
            global current_scroll
            current_scroll += config.settings["devices"][self.board]["effect_opts"]["Wave"]["speed"] + 1
            speed = int( current_scroll )
            current_scroll -= speed
            self.wave_wipe_count += speed
        return output
    ...
    def visualize_power(self, y):
        ...
        for index in self.power_indexes:
            ...
            output[1, index] = int(config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Power"]["color_flash"]][1]*self.power_brightness)
            output[2, index] = int(config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Power"]["color_flash"]][2]*self.power_brightness)
    ...
    def visualize_pulse(self, y):
        ...
        if self.current_freq_detects["beat"]:
            ...
            #output = np.multiply(self.prev_output,config.settings["devices"][self.board]["effect_opts"]["Beat"]["decay"])
            #output = np.zeros((3,config.settings["devices"][self.board]["configuration"]["N_PIXELS"]))
            #output[0][:]=config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Pulse"]["color"]][0]
            #output[1][:]=config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Pulse"]["color"]][1]
            #output[2][:]=config.settings["colors"][config.settings["devices"][self.board]["effect_opts"]["Pulse"]["color"]][2]
        else:
          y = y**config.settings["devices"][self.board]["effect_opts"]["Pulse"]["sensitivity"]
          y = np.copy(interpolate(y, config.settings["devices"][self.board]["configuration"]["N_PIXELS"] // 2))
          signal_processers[self.board].common_mode.update(y)
          diff = y - self.prev_spectrum
          diff = np.multiply( diff, 0.9 ) #should be decay
          y += diff
          self.prev_spectrum = np.copy(y)
          ...
          r = signal_processers[self.board].r_filt.update(y - signal_processers[self.board].common_mode.value) 
          r = np.array([j for i in zip(r,r) for j in i])
          g = signal_processers[self.board].g_filt.update(y - signal_processers[self.board].common_mode.value) 
          g = np.array([j for i in zip(g,g) for j in i])
          b = signal_processers[self.board].b_filt.update(y - signal_processers[self.board].common_mode.value) 
          b = np.array([j for i in zip(b,b) for j in i])        
          output = np.array([self.multicolor_modes[config.settings["devices"][self.board]["effect_opts"]["Pulse"]["color_mode"]][0][:config.settings["devices"][self.board]["configuration"]["N_PIXELS"]]*r,
                             self.multicolor_modes[config.settings["devices"][self.board]["effect_opts"]["Pulse"]["color_mode"]][1][:config.settings["devices"][self.board]["configuration"]["N_PIXELS"]]*g,
                             self.multicolor_modes[config.settings["devices"][self.board]["effect_opts"]["Pulse"]["color_mode"]][2][:config.settings["devices"][self.board]["configuration"]["N_PIXELS"]]*b])
          #output = (2 * output)
    ...
    def visualize_gradient(self):
        ...
        global current_roll
        current_roll += config.settings["devices"][self.board]["effect_opts"]["Gradient"]["roll_speed"]*(-1 if config.settings["devices"][self.board]["effect_opts"]["Gradient"]["reverse"] else 1)
        truncated_roll = int( current_roll )
        current_roll -= truncated_roll
        self.multicolor_modes[config.settings["devices"][self.board]["effect_opts"]["Gradient"]["color_mode"]] = np.roll(
                           ...
                           truncated_roll,
                           axis=1)
        output = gaussian_filter1d(output, sigma=config.settings["devices"][self.board]["effect_opts"]["Gradient"]["blur"])
    ...
    def visualize_fade(self):
        ...
        current_roll += config.settings["devices"][self.board]["effect_opts"]["Fade"]["roll_speed"]*(-1 if config.settings["devices"][self.board]["effect_opts"]["Fade"]["reverse"] else 1)
        truncated_roll = int( current_roll )
        current_roll -= truncated_roll
...
# From :https://nicholastsmith.wordpress.com/2017/08/10/poe-ai-part-4-real-time-screen-capture-and-plumbing/
...
class ScreenViewer:
    ...
    def ScreenUpdateT(self):
        ...
        while self.cl:
            ...
            self.its = time.time()
            self.mut.release()
            fps = config.settings["configuration"]["SCREENGRAB_MAX_FPS"]
            time.sleep(max(1./fps - (time.time() - t1), 0))
			
    	
    def adjust_gamma( self, im, gamma):
    ...
    def adjust_channel_gamma( self, im, r, g, b ):
        """Fast gamma correction with PIL's image.point() method"""
        invert_gamma_r = 1.0/r
        invert_gamma_g = 1.0/g
        invert_gamma_b = 1.0/b
