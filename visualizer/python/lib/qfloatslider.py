...
class QFloatSlider(QtWidgets.QSlider):
    ...
    #note - was int(round(a/self.step)), but int to float adds the min value, which seemed like it was bogus when going abck and forth - number would grow
    def _float_to_int(self, a):
        return int(round(( a- self.min_value)/self.step))
